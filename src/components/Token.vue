  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        state.wbnbBal = parseFloat(formatEther(amountsOut[1])).toFixed(6);

        const amountsOut2 = await router.getAmountsOut(parseEther("1.0"), [
          config.WBNB_ADDRESS,
          config.BUSD_ADDRESS,
        ]);

        state.busdX = parseFloat(formatEther(amountsOut2[1])).toFixed(6);
      } catch (e) {
        console.log(e);
      }

      try {
        const totalTokenSupply = await hold.totalSupply();
        state.totalSupply = parseFloat(formatEther(totalTokenSupply)).toFixed(
          3
        );

        const toMint = await hold.toMint();
        state.toMint = parseFloat(formatEther(toMint)).toFixed(3);
      } catch (e) {
        console.log(e);
      }

      state.totalMarketCap = parseFloat(
        state.wbnbBal * state.totalSupply
      ).toFixed(3);
    });
